...
class DiscriminatorLogReg:
    ...
    def optimize_and_save_hyperparams(self, X_train, y_train, X_val, y_val, n_iter=50):
        ...
        param_space = {'C': Real(1e-4, 1e2, prior='log-uniform'), 'penalty': Categorical(['l1', 'l2']), 'class_weight': Categorical(class_weight_options)}
        lr = LogisticRegression(random_state=self.random_state, max_iter=2000, solver='saga')
        print(f"BayesSearchCV (LogReg): testing {n_iter} parameter combinations (Bayesian optimization)...")
        bayes_search = BayesSearchCV(lr, param_space, n_iter=n_iter, cv=5, scoring='neg_log_loss', n_jobs=1, verbose=0, random_state=self.random_state, return_train_score=True)
    ...
    def train_loop(self, train_dl, val_dl, epochs, lr=1e-3):
        ...
        lr_params = {'random_state': self.random_state, 'max_iter': 2000, 'solver': 'saga'}
        if self.best_params:
            lr_params.update(self.best_params)
        base_lr = LogisticRegression(**lr_params)
        base_lr.fit(X_train, y_train)
        method = "isotonic" if len(y_val) > 1000 else "sigmoid"
        self.model = CalibratedClassifierCV(base_lr, method=method, cv="prefit")
